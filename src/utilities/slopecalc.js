...
const sortFirst = (a, b) => sortNumeric(a[0], b[0]);
...
const normalizeWithOrig = value => [normalizeValue(value), value];

const calcParts = (from, to) => {
...
const createTransformCalc = (from, to) =>
  from.size
    .map((fromProp, i) => {
      ...
      const { slope, yIntercept } = calcParts(
        { bp: from.bp, size: fromValue },
        { bp: to.bp, size: toValue }
      );
    ...
    Object.keys(breakpoints[prop])
      ...
      .reduce(
        (prev, [bp, origBp]) => {
          ...
          return (breakById[prev.bp][prev.index].to =
            breakById[bp][index].from);
        },
        { bp: 0 }
      );
  });
  ...
  return breaks.sort(sortNumeric).reduce((acc, bp) => {
    let styles = breakById[bp].reduce((css, data) => {
      ...
      if (!data.from) {
        value =
          data.prop === 'transform'
            ? createTransformClamp(data.to.size)
            : `${data.to.size}px`;
      } else if (!data.to) {
        value =
          data.prop === 'transform'
            ? createTransformClamp(data.from.size)
            : `${data.from.size}px`;
      } else {
        value = (data.prop === 'transform' ? createTransformCalc : createCalc)(
          data.from,
          data.to
        );
      }
...
export default function expose(arg) {
  ...
  if (Array.isArray(arg)) {
    return bp =>
      slopeCalc(arg.reduce((acc, prop) => ({ [prop]: bp, ...acc }), {}));
  }
